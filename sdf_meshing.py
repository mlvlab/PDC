...
def create_mesh(model, filename, subject_idx=0, N=128, max_batch=64 ** 3, offset=None, scale=None,level=0.0,
        get_color=False,latent_z=None, embedding_geo=None,embedding_sem=None, extractor=None):
    start = time.time()
    ...
    while head < num_samples:
        ...
        if subject_idx == -1:
            ...
            pdb.set_trace()
            samples[head : min(head + max_batch, num_samples), 3] = (
                model.inference(sample_subset,latent_z,embedding_geo,embedding_sem,extractor)
                .squeeze()#.squeeze(1)
                .detach()
                .cpu()
            )
    ...
    if not get_color:
        ...
        convert_sdf_samples_with_color_to_ply(
            ...
            scale,
            level,
            latent_z,embedding_geo,embedding_sem,extractor
        )


def get_mesh_color(mesh_points,model,latent_z,embedding_geo,embedding_sem,extractor):
    ...
    while head < num_samples:
        ...
        mesh_colors[head : min(head + max_batch, num_samples), 0:3] = (
            model.get_template_coords(sample_subset,latent_z,embedding_geo,embedding_sem,extractor)
            .squeeze()#.squeeze(1)
            .detach()
            .cpu()
        )
...
def convert_sdf_samples_with_color_to_ply(
    ...
    scale=None,
    level=0.0,
    latent_z=None, embedding_geo=None,embedding_sem=None,extractor=None):
    """
    ...
    mesh_colors = get_mesh_color(mesh_points,model,latent_z,embedding_geo,embedding_sem,extractor)
